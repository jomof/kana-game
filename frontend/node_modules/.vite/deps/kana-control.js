...
// node_modules/@patdx/kuromoji/build/index.mjs
...
// node_modules/kana-control/tokenize.js
var loader = {
...
function getTokenizer() {
  if (!tokenizerPromise) {
    tokenizerPromise = new TokenizerBuilder_default({ loader }).build();
  }
  return tokenizerPromise;
}
var defaultTokenize = async (text) => {
  try {
...
var tokenizeImpl = defaultTokenize;
var tokenize = (text) => tokenizeImpl(text);

// node_modules/kana-control/augment.js
...
async function augmentDesuDaTokens(tokens) {
  ...
  if (desuIndex !== tokens.length - 1 && (desuIndex !== tokens.length - 2 || tokens[tokens.length - 1].pos !== "記号" && tokens[tokens.length - 1].pos !== "名詞")) {
  ...
  const replacement = tokens.map((t, i) => {
    if (i === desuIndex)
      return "だ";
...
var augmentDropWatashiHa = makeTokenAugmenter(
...
function makeReadingModifierAugmenter(surfaceForm, newReading) {
  return async (tokens) => {
...
var tokenAugmenters = [
  ...
  async (tokens) => {
    ...
    if (daIndex === -1)
      return [];
    if (daIndex !== tokens.length - 1 && (daIndex !== tokens.length - 2 || tokens[tokens.length - 1].pos !== "記号" && tokens[tokens.length - 1].pos !== "名詞")) {
      return [];
    }
    if (daIndex > 0 && tokens[daIndex - 1].pos !== "名詞")
      return [];
    const replacement = tokens.map((t, i) => i === daIndex ? "です" : t.surface_form).join(" ");
    const replacementTokens = await tokenize(replacement);
  ...
  async (tokens) => {
    ...
    if (lastIdx >= 1) {
      ...
      if (last2.pos === "動詞" && last2.surface_form === "いる" && prev.surface_form.match(/^(て|で)$/)) {
        const out2 = tokens.map((t, i) => {
          if (i === lastIdx)
            return "る";
          return t.surface_form;
        }).join(" ");
        return [await tokenize(out2)];
      }
    }
    if (lastIdx < 0)
      return [];
  ...
  async (tokens) => {
    for (let i = 0; i < tokens.length - 1; i++) {
      const t0 = tokens[i];
      const t1 = tokens[i + 1];
      if (t0.surface_form === "と" && t0.pos === "助詞" && t1.pos === "動詞" && (t1.basic_form === "思う" || t1.surface_form === "思う")) {
        const out = tokens.map((t, j) => {
          if (j === i)
            return "って";
          return t.surface_form;
  ...
  async (tokens) => {
    for (let i = 0; i < tokens.length - 1; i++) {
      const t0 = tokens[i];
      const t1 = tokens[i + 1];
      if (t0.surface_form === "って" && t1.pos === "動詞" && (t1.basic_form === "思う" || t1.surface_form === "思う")) {
        const out = tokens.map((t, j) => {
          if (j === i)
            return "と";
          return t.surface_form;
  ...
  async (tokens) => {
    ...
    const replaced = tokens.map((t, i) => i === lastIdx ? stem : t.surface_form).join(" ");
  ...
  async (tokens) => {
    ...
    if (!polite)
      return [];
    const replaced = tokens.map((t, i) => i === lastIdx ? polite : t.surface_form).join(" ");
    return [await tokenize(replaced)];
  },
...
function toLexicalKey(tokens) {
  return JSON.stringify(tokens.map((t) => `${t.surface_form}(${t.reading})`));
}
async function augmentTokenGroups(initialGroups) {
...
async function makeQuestion(english, japanese) {
