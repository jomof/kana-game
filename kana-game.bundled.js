...
function SAFE_HEAP_STORE(dest, value, bytes, isFloat) {
 ...
 setValue(dest, value, getSafeHeapType(bytes, isFloat));
 return value;
}
...
function createExportWrapper(name, fixedasm) {
 return function() {
  ...
  assert(true, "native function `" + displayName + "` called after runtime exit (use NO_EXIT_RUNTIME to keep it alive after main() exits)");
  if (!asm[name]) {
...
Module["_mecab_new2"] = createExportWrapper("mecab_new2");

Module["_mecab_sparse_tostr3"] = createExportWrapper("mecab_sparse_tostr3");
...
var lib;
var instance;
const libPromise = Module({ locateFile });
libPromise.then(x => { 
    lib = x;
...
let KanaGame = class KanaGame extends i {
    ...
    firstUpdated() {
        const input = this.shadowRoot?.getElementById('kana-input');
        if (input) {
...
__decorate([
    n()
], KanaGame.prototype, "name", void 0);
__decorate([
    n({ type: Number })
], KanaGame.prototype, "count", void 0);
__decorate([
    n({ type: Boolean })
], KanaGame.prototype, "mecabInitialized", void 0);
__decorate([
    n({ type: String })
